function [ outVal,outGr,outHes ] = mojaFullHessian1( x0,VGH )
...
if VGH(3) > 0
   ...
   for i=2:n-1
       for j=i:n-1
           ...
           outHes(j,i) = -8*(n-j+1)*(x0(1)-3)+48*postQuadPref(j);
       end
   end
   ...
   for i=2:n
       ...
       outHes(i,1) = outHes(i,i)-8*postSumPref(i);
   end
   for i=2:n
       outHes(n,i) = -8*(x0(1)-3 - 6*pref(n).^2);
       outHes(i,n) = -8*(x0(1)-3 - 6*pref(n).^2);
   end
   outHes(1,n) = -8*(x0(1)-3+pref(n)-6*pref(n).^2);
   outHes(n,1) = -8*(x0(1)-3+pref(n)-6*pref(n).^2);
