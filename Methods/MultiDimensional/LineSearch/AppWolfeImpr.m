function [ outT, outX, evalNumbers ] = AppWolfeImpr( functionName, params)    
...
%   ------------------      *******************        ------------------
    ...
    while 1
        ...
        derPhiC = derPhiC'*dir';                    % derivative of Phi(c) in current point 
...
function [c, phiC, evalNumbers] = initial(functionName, x0, val0, der0, derPhi0, dir, k, cOld)
    ...
    if 1 % currently is in use
        ...
        if val0 - phiR + R*derPhi0 < 0 %&& phiR < val0
                    
            % computes minimum of interpolation function that 
            % matches val0, der0, phiR, R
